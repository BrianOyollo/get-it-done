...
from ..models import  Report, Subcategory, ReportStatus, ModeratorAction
    ...
    try:
        ...
        if not user.is_superuser:
            if user.moderator.subcategory != report.subcategory:
                return HttpResponse(f"You can only updates report under {user.moderator.subcategory}", status=403)
