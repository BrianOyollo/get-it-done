...
def new_subcategory(request, payload:NewSubcategorySchema): 
    if not request.user.is_superuser:
        raise HttpError(403, 'Only admins can add new subcategories.')

    try:
        category = get_object_or_404(Category, pk=payload.category)
        subcategory, created = Subcategory.objects.get_or_create(
            category=category, description=payload.description, subcategory=payload.subcategory)
        return subcategory
...
def edit_subcategory(request, subcategory_id:int, payload:EditSubcategorySchema):
    if not request.user.is_superuser:
        raise HttpError(403, 'Only admins can edit subcategories')
...
def delete_subcategory(request, subcategory_id:int):
    if not request.user.is_superuser:
        raise HttpError(403, 'Only admins can delete subcategories')
