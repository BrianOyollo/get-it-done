...
from core.models import Moderator, ModeratorApplication, ModeratorAction, Subcategory
...
def moderators(request):
    if not request.user.is_superuser:
        raise HttpError(403, "Request denied: Only admins are can view moderators")
...
def moderator_applications(request):
    if not request.user.is_superuser:
        raise HttpError(403, 'Request denied: You are not allowed to view moderator applications')
...
def moderator(request, moderator_id:int):
    if not request.user.is_superuser:
        raise HttpError(403, "Request denied: Only admins can view a moderator")
...
def delete_moderator(request, moderator_id:int):
    if not request.user.is_superuser:
        raise HttpError(403, "Request denied: Only admins can delete a moderator")
...
def new_moderator_application(request, payload:NewModeratorApplicationSchema):
    if not request.user.is_authenticated:
        raise HttpError(403, "Request denied: You must be logged in to apply as a moderator")
    
    if request.user.is_moderator:
        raise HttpError(403, "Request denied: You are already a moderator")
...
def approve_moderator_application(request, application_id:int):
    if not request.user.is_superuser:
        raise HttpError(403, "Request denied: Only admins can approve moderator applications")
