...
def new_report_status(request, payload:NewReportStatusSchema): 
    if not request.user.is_superuser:
        raise HttpError(403, 'Request denied: Only admins can create report statuses')
...
def edit_report_status(request, status_id:int, payload:EditReportStatusSchema):
    if not request.user.is_superuser:
        raise HttpError(403, 'Request denied: Only admins can edit report statuses')
...
def delete_report_status(request, status_id:int):
    if not request.user.is_superuser:
        raise HttpError(403, 'Request denied: Only admins can delete report statuses')
